...
base = Path('output/annaheaner-live-site-lite')
if base.exists():
    shutil.rmtree(base)
...
# source images from package and attachment
...
for name in selected:
    ...
    dst = assets / name.rsplit('.',1)[0] + '.jpg'
...
secondary = [n for n in image_names if n != hero]
...
for i, title in enumerate(story_titles):
    ...
    story_cards.append(f'''<article class="story"><img src="assets/images/{img}" alt="{title}" loading="lazy"><div class="story-body"><div class="meta">Travel note</div><h3>{title}</h3><p>{story_texts[i % len(story_texts)]}</p></div></article>''')

gallery_imgs = image_names[:16]
gallery = '\n'.join([f'<button class="tile" data-image="assets/images/{n}" aria-label="Open {n}"><img src="assets/images/{n}" alt="Photo from the journal" loading="lazy"></button>' for n in gallery_imgs])
...
<script>
(()=>{{
const r=document.documentElement,t=document.querySelector('[data-theme-toggle]');let theme=matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';r.setAttribute('data-theme',theme);t.onclick=()=>{{theme=theme==='dark'?'light':'dark';r.setAttribute('data-theme',theme)}};
const l=document.getElementById('lightbox'),i=document.getElementById('lightboxImage'),c=document.getElementById('close');document.querySelectorAll('.tile').forEach(b=>b.onclick=()=>{{i.src=b.dataset.image;l.classList.add('open')}});c.onclick=()=>{{l.classList.remove('open');i.src=''}};l.onclick=e=>{{if(e.target===l)c.onclick()}};
}})();
</script>
</body>
</html>
